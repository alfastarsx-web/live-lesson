...
async function signIn(login, password) {
  ...
  const entered = String(login).trim();
  const testDomain = process.env.LOCAL_TEST_LOGIN_DOMAIN || '';
  const identifier = testDomain && /^[a-z][a-z0-9-]*$/i.test(entered)
    ? `${entered.toLowerCase()}@${testDomain}` : entered;
  const body = identifier.includes('@')
    ? { email: identifier, password }
    : { phoneNumber: normalizePhone(identifier), password };
  ...
  if (!roles.length) {
    diag();
    return { ok: false, status: 403, message: 'Hisob roli aniqlanmadi' };
  }
  ...
  // Xona nomi uchun xavfsiz ko'rinishga keltiramiz
  const safeId = String(id).toLowerCase().replace(/[^a-z0-9-]+/g, '').slice(0, 50);
  if (safeId.length < 1) return { ok: false, status: 502, message: 'Foydalanuvchi aniqlanmadi' };

  const isMentor = roles.includes('mentor') || roles.includes('admin');
  // Mentor bergan bir martalik kod bilan kirgan bo'lsa, sahifa parol so'raydi
  const mustChangePassword = data.mustChangePassword === true
    || data.data?.mustChangePassword === true;

  return {
    ok: true, id: safeId, name: String(name).slice(0, 40), roles, isMentor, token,
    mustChangePassword,
  };
}
